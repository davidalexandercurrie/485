...
#elif IS_DUE()
static void dueAudioOutput() {
  dacc_set_channel_selection(DACC_INTERFACE, AUDIO_CHANNEL_1);    
  dacc_write_conversion_data(DACC_INTERFACE, (int)output_buffer.read());
}

#elif IS_ESP32()
static void IRAM_ATTR esp32AudioOutput() {
	int out32=(int)output_buffer.read();
...
static void startAudioStandard()
{
...
#elif IS_DUE()
  analogWriteResolution(12);
  Timer6.attachInterrupt(dueAudioOutput).setFrequency(AUDIO_RATE).start();
  analogWrite(AUDIO_CHANNEL_1_PIN,0);
  analogWrite(AUDIO_CHANNEL_2_PIN,0);
  ...
  timerAttachInterrupt(timer, &esp32AudioOutput, true);
	// zc - 45 microseconds is ok for now
